...
import (
	...
	"github.com/gnaoh1379/unioffice/schema/urn/schemas_microsoft_com/office/powerpoint"
	"github.com/gnaoh1379/unioffice/schema/urn/schemas_microsoft_com/office/word"
)
...
func (m *CT_Shapetype) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if m.MasterAttr != nil {
		start.Attr = append(start.Attr, xml.Attr{Name: xml.Name{Local: "o:master"},
			Value: fmt.Sprintf("%v", *m.MasterAttr)})
	}
	if m.HrefAttr != nil {
	...
	if m.Complex != nil {
		secomplex := xml.StartElement{Name: xml.Name{Local: "o:complex"}}
		e.EncodeElement(m.Complex, secomplex)
	}
...
func (m *CT_Shapetype) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	...
	for _, attr := range start.Attr {
		...
		if attr.Name.Space == "urn:schemas-microsoft-com:office:office" && attr.Name.Local == "master" {
			parsed, err := attr.Value, error(nil)
			if err != nil {
				return err
			}
	...
	for {
		...
		case xml.StartElement:
			...
			case xml.Name{Space: "urn:schemas-microsoft-com:office:powerpoint", Local: "textdata"}:
				...
				m.EG_ShapeElements = append(m.EG_ShapeElements, tmpshapeelements)
			case xml.Name{Space: "urn:schemas-microsoft-com:office:office", Local: "complex"}:
				m.Complex = NewOfcComplex()
				if err := d.DecodeElement(m.Complex, &el); err != nil {
					return err
				}
			default:
				unioffice.Log("skipping unsupported element on CT_Shapetype %v", el.Name)
...
func (m *CT_Shapetype) Validate() error {
	return m.ValidateWithPath("CT_Shapetype")
}
...
func (m *CT_Shapetype) ValidateWithPath(path string) error {
	...
	if m.Complex != nil {
		if err := m.Complex.ValidateWithPath(path + "/Complex"); err != nil {
			return err
		}
	}
