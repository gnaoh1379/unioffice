...
func (m *CT_PolyLine) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if m.PointsAttr != nil {
		start.Attr = append(start.Attr, xml.Attr{Name: xml.Name{Local: "points"},
			Value: fmt.Sprintf("%v", *m.PointsAttr)})
	}
	if m.HrefAttr != nil {
	...
	for {
		...
		case xml.StartElement:
			...
			case xml.Name{Space: "urn:schemas-microsoft-com:office:powerpoint", Local: "textdata"}:
				...
				m.EG_ShapeElements = append(m.EG_ShapeElements, tmpshapeelements)
			default:
				unioffice.Log("skipping unsupported element on CT_PolyLine %v", el.Name)
...
func (m *CT_PolyLine) Validate() error {
	return m.ValidateWithPath("CT_PolyLine")
}
