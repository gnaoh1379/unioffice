...
func (m *CT_RoundRect) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if m.ArcsizeAttr != nil {
		start.Attr = append(start.Attr, xml.Attr{Name: xml.Name{Local: "arcsize"},
			Value: fmt.Sprintf("%v", *m.ArcsizeAttr)})
	}
	if m.HrefAttr != nil {
	...
	for {
		...
		case xml.StartElement:
			...
			case xml.Name{Space: "urn:schemas-microsoft-com:office:powerpoint", Local: "textdata"}:
				...
				m.EG_ShapeElements = append(m.EG_ShapeElements, tmpshapeelements)
			default:
				unioffice.Log("skipping unsupported element on CT_RoundRect %v", el.Name)
...
func (m *CT_RoundRect) Validate() error {
	return m.ValidateWithPath("CT_RoundRect")
}
