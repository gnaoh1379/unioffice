...
func (m *CT_Arc) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if m.StartAngleAttr != nil {
		start.Attr = append(start.Attr, xml.Attr{Name: xml.Name{Local: "startAngle"},
			Value: fmt.Sprintf("%v", *m.StartAngleAttr)})
	}
	if m.EndAngleAttr != nil {
		start.Attr = append(start.Attr, xml.Attr{Name: xml.Name{Local: "endAngle"},
			Value: fmt.Sprintf("%v", *m.EndAngleAttr)})
	}
	if m.HrefAttr != nil {
...
func (m *CT_Arc) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	...
	for _, attr := range start.Attr {
		...
		if attr.Name.Local == "endAngle" {
			parsed, err := strconv.ParseFloat(attr.Value, 64)
			if err != nil {
				return err
			}
			m.EndAngleAttr = &parsed
			continue
		}
		...
		if attr.Name.Local == "startAngle" {
			parsed, err := strconv.ParseFloat(attr.Value, 64)
			if err != nil {
				return err
			}
			m.StartAngleAttr = &parsed
			continue
		}
	...
	for {
		...
		case xml.StartElement:
			...
			case xml.Name{Space: "urn:schemas-microsoft-com:office:powerpoint", Local: "textdata"}:
				...
				m.EG_ShapeElements = append(m.EG_ShapeElements, tmpshapeelements)
			default:
				unioffice.Log("skipping unsupported element on CT_Arc %v", el.Name)
...
func (m *CT_Arc) Validate() error {
	return m.ValidateWithPath("CT_Arc")
}
